...
const checks = [
  ...
  ['one-light-year milestone hands off to the existing Galaxy view', viewerLifecycle.includes('onInterstellarHandoff') && viewerLifecycle.includes('solarZoomStep === lastSolarZoomStep') && component.includes('setSpaceEntryLevel("galaxy")') && component.includes('initialLevel={spaceEntryLevel}') && spaceExplorer.includes('initialLevel: SpaceLevel') && spaceExplorer.includes('useState<SpaceLevel>(initialLevel)')],
