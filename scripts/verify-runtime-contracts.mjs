...
const checks = [
  ...
  ['Earth reference waits until the native globe is visually small', viewScale.includes('EARTH_REFERENCE_MIN_DISTANCE_M = 0.25 * AU_METERS') && component.includes('showEarthReference: earthScene.earthReferenceVisible') && celestialBridge.includes('if (args.showEarthReference)')],
  ...
  ['Full Solar uses the current JPL body bounds instead of another distance ceiling', celestialBridge.includes('Cesium.BoundingSphere.fromPoints(solarFramePoints)') && celestialBridge.includes('getSolarFrame()') && viewerLifecycle.includes('getSolarFrame?: () => SolarFrameSnapshot | null') && viewerLifecycle.includes('applySolarFrame(FULL_SOLAR_CONTEXT_DISTANCE_M)')],
  ['Full Solar holds four deliberate wheel steps and exits with reverse hysteresis', viewScale.includes('FULL_SOLAR_DWELL_STEPS = 4') && viewScale.includes('FULL_SOLAR_EXIT_DISTANCE_M = 24 * AU_METERS') && viewerLifecycle.includes('fullSolarDwellStep < FULL_SOLAR_DWELL_STEPS') && viewerLifecycle.includes('nextLogicalHeight <= FULL_SOLAR_EXIT_DISTANCE_M') && component.includes('OUTER READY')],
  ...
  ['satellites freeze after live propagation but persist as fading spatial context', viewScale.includes('SATELLITE_LIVE_PROPAGATION_MAX_HEIGHT_M = 120_000_000') && satelliteRenderer.includes("import { SATELLITE_LIVE_PROPAGATION_MAX_HEIGHT_M } from '@/lib/view-scale'") && satelliteRenderer.includes('const livePropagation =') && satelliteRenderer.includes('pointCollection.show = visible') && satelliteRenderer.includes('5_000_000_000, 0.04') && !satelliteRenderer.includes('deepSolarContext')],
