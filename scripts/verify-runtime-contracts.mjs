...
const checks = [
  ...
  ['Solar frame includes complete orbit paths and locks a stable ecliptic view', celestialBridge.includes('solarFramePoints.push(Cesium.Cartesian3.clone(orbitPosition))') && celestialBridge.indexOf('solarFramePoints.push(Cesium.Cartesian3.clone(orbitPosition))') < celestialBridge.indexOf('const bounds = Cesium.BoundingSphere.fromPoints(solarFramePoints)') && viewerLifecycle.includes('const setSolarCameraLock') && viewerLifecycle.includes('controller.enableRotate = !locked') && viewerLifecycle.includes('controller.enableZoom = !locked')],
