...
const checks = [
  ...
  ['Earth camera and cislunar bodies retain the shared logical display contract', viewScale.includes('logicalToDisplayDistanceM') && viewScale.includes('displayToLogicalDistanceM') && viewScale.includes('CELESTIAL_COMPRESSION_START_M = 60_000_000') && celestialBridge.includes('logicalToDisplayDistanceM')],
  ...
  ['Solar reveal at one AU frames every current JPL body bound', viewScale.includes('SOLAR_HANDOFF_DISTANCE_M = AU_METERS') && viewScale.includes('FULL_SOLAR_CONTEXT_DISTANCE_M = SOLAR_HANDOFF_DISTANCE_M') && celestialBridge.includes('Cesium.BoundingSphere.fromPoints(solarFramePoints)') && celestialBridge.includes('getSolarFrame()') && viewerLifecycle.includes('nextLogicalHeight >= FULL_SOLAR_CONTEXT_DISTANCE_M') && viewerLifecycle.includes('applySolarFrame(solarZoomStep)')],
  ['Solar navigation uses a symmetric four-to-one dead-scroll gear', viewScale.includes('SOLAR_DEAD_SCROLL_TICKS = 4') && viewerLifecycle.includes('solarDeadScrollTick >= SOLAR_DEAD_SCROLL_TICKS') && viewerLifecycle.includes('solarZoomStep + 1') && viewerLifecycle.includes('solarZoomStep -= 1') && component.includes('INPUT ${earthCamera.solarDeadScrollTick}/${SOLAR_DEAD_SCROLL_TICKS}')],
