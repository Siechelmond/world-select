...
const checks = [
  ...
  ['celestial bridge preserves cislunar room before compressed heliocentric geometry', celestialBridge.includes('SOLAR_DISPLAY_ONE_AU_M = 5_000_000_000') && celestialBridge.includes('compressedHeliocentricPosition') && !celestialBridge.includes('displayRadius(distanceAu, cameraHeight)')],
  ...
  ['Earth Moon keeps true 384400 km data while cislunar display compresses only after GEO', spaceModel.includes('{ name: "Moon", radiusKm: 1737.4, orbitalRadiusKm: 384400') && celestialBridge.includes('GEO_REFERENCE_ORBIT_RADIUS_M = 42_164_000') && celestialBridge.includes('MOON_DISPLAY_ORBIT_RADIUS_M = 150_000_000') && celestialBridge.includes('cislunarDisplayRadiusMeters') && celestialBridge.includes('trueOrbitalRadiusKm: moon.orbitalRadiusKm')],
  ...
  ['high-orbit wheel zoom is damped without changing ground controls', viewerLifecycle.includes('ORBIT_WHEEL_DAMPING_HEIGHT_M = 20_000_000') && viewerLifecycle.includes('MAX_ORBIT_WHEEL_LOG_STEP = 0.12') && viewerLifecycle.includes("addEventListener('wheel', onOrbitScaleWheel") && viewerLifecycle.includes('currentHeight * Math.exp(logStep)') && viewerLifecycle.includes("removeEventListener('wheel', onOrbitScaleWheel, true)")],
