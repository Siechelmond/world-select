...
const checks = [
  ...
  ['Earth and Solar scale telemetry derive from the existing camera owner', component.includes('function formatEarthDistance') && component.includes('const earthDistanceLabel = useMemo(() => formatEarthDistance(cameraHeight)') && component.includes('earthCamera.solarFrame ? "SCALE" : "DIST"')],
  ...
  ['one-light-year milestone keeps Galaxy inside the Earth zoom path', viewerLifecycle.includes('onInterstellarHandoff') && viewerLifecycle.includes('solarZoomStep === lastSolarZoomStep') && component.includes('setEarthGalaxyFrame(true)') && component.includes('viewMode === "earth" && earthGalaxyFrame') && component.includes('<GalaxyView') && !component.includes('setSpaceEntryLevel("galaxy")') && spaceExplorer.includes('export function GalaxyView')],
  ['Space mode remains a manual orbit entry', component.includes('initialLevel="orbit"') && component.includes('viewerLifecycleRef.current?.leaveSolarFrame()')],
  ['Orbit visibility cannot change the Solar camera frame and the view stays ecliptic-locked', !celestialBridge.includes('solarFramePoints.push(Cesium.Cartesian3.clone(orbitPosition))') && celestialBridge.indexOf('const bounds = Cesium.BoundingSphere.fromPoints(solarFramePoints)') < celestialBridge.indexOf('if (args.showOrbits)') && celestialBridge.includes('depthFailMaterial: material') && viewerLifecycle.includes('const setSolarCameraLock') && viewerLifecycle.includes('controller.enableRotate = !locked') && viewerLifecycle.includes('controller.enableZoom = !locked')],
  ...
  ['Solar overview keeps moon points selectable without label crowding', celestialBridge.includes('showLabel:\n          !args.solarFrameActive') && celestialBridge.includes('showLabel || selected')],
