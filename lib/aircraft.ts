...
type AdsbAircraft = {
  ...
  alt_baro?: number | "ground"; alt_geom?: number; gs?: number; track?: number;
  squawk?: string; seen?: number; emergency?: string; category?: string;
};

type AdsbResponse = { ac?: AdsbAircraft[]; now?: number; total?: number };
...
export async function fetchAircraftNear(query: AircraftQuery, signal?: AbortSignal): Promise<SpatialEntity[]> {
  const radiusNm = Math.max(25, Math.min(250, Math.round(query.radiusNm ?? 250)));
  ...
  if (!response.ok) throw new Error(`Aircraft proxy returned HTTP ${response.status}`);
  ...
  return (payload.ac ?? []).flatMap((aircraft, index) => {
    ...
    return [{
      ...
      source: { id: "adsb-lol", label: "ADSB.lol community ADS-B", url: "https://www.adsb.lol/" },
      properties: {
