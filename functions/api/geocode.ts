...
function heightFor(row: NominatimRow) {
  ...
  if (/(house|building|address)/.test(type)) return 2_500;
  if (/suburb|neighbourhood|quarter/.test(type)) return 6_000;
  if (/village|hamlet/.test(type)) return 8_000;
  if (/town/.test(type)) return 12_000;
  if (/city|municipality/.test(type)) return 20_000;
  ...
  if (box?.length === 4 && box.every(Number.isFinite)) {
    ...
    return Math.min(900_000, Math.max(6_000, Math.max(latSpan, lonSpan) * 111_000 * 1.15));
  }
  return 180_000;
}
